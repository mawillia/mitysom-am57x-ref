...
vivado -mode batch -source script/gen_pcie_dma_example_project.tcl

This will create a project at ./pcie_dma_example/pcie_dma_example.xpr, using external references for the provided
source files.
...
vivado -mode batch -source ./script/build_pcie_dma_example_bitstream_batch.tcl

This will create an output folder ./pcie_dma_example_output and will place the build report files as well as a generated bitstream.
...
In addition, common ip files (provided by Critical Link) are included from ../ip (above the pcie_dma_example folder).
...
├── script
│   └── gen_pcie_dma_example_project.tcl      // Script to build Vivado Project
└── src
    ├── constraints                 // Vivado Timing Constraints
    │   ├── pcie_dma_example_top.xdc
    │   └── xilinx_pcie_7x_ep_x2g2.xdc
    └── hdl                         // Project HDL source files.
        ├── pcie_dma_example_top.vhd
```
